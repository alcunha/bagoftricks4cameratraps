...
class BalancedGroupSoftmax:
  def __init__(self,
    ...
    dataset_df = self._load_dataset(dataset_json)
    self._generate_binlabel_idx(dataset_df)
    self._generate_tables()

  def _load_dataset(self, dataset_json):
  ...
  def _generate_binlabel_idx(self, dataset_df):
    ...
    for categ in categories:
      instances_count = len(dataset_df[dataset_df.category_id == categ])
  ...
  def _generate_tables(self):
    ...
    for label, binlabel in self.label2binlabel.items():
      ...
      for i in range(self.n_groups + 1):
        self.map_tables[i][binlabel[i]][label] = 1.0
  ...
  def process_label(self, label):
    ...
    labels = [tf.squeeze(
                  tf.matmul(tf.constant(self.map_tables[i], dtype=tf.float32),
                            label), 1)
              for i in range(self.n_groups+1)]
