...
from eval_utils import get_cls_bin_lists, save_results_to_json
...
flags.DEFINE_string(
    ...
    help=('Json file name where the results will be stored.'))
...
def predict_classifier(model, dataset):
  ...
  return labels, predictions, file_names

def save_results(acc_all, acc_bins, f1_score_cls, conf_matrix):
  data = {}
  data['acc_all'] = acc_all
  data['acc_bins'] = acc_bins
  data['f1_score_cls'] = f1_score_cls.tolist()
  data['confusion_matrix'] = conf_matrix.tolist()

  save_results_to_json(data, FLAGS.results_file, FLAGS.model_weights)

def eval_classifier(model, dataset, cls_bins):
  labels, predictions, file_names = predict_classifier(model, dataset)
  preds_df = pd.DataFrame({'labels': labels,
                           'predictions': predictions,
                           'file_names': file_names})

  acc_all = accuracy_score(labels, predictions)
  conf_matrix = confusion_matrix(labels, predictions)
  _, _, f1_score_cls, _ = precision_recall_fscore_support(labels, predictions)

  acc_bins = {}
  for bin_n in cls_bins:
    instances = preds_df[preds_df.labels.isin(cls_bins[bin_n])]
    labels_bin = instances['labels'].tolist()
    predictions_bin = instances['predictions'].tolist()
    acc = accuracy_score(labels_bin, predictions_bin)
    acc_bins[bin_n] = acc

  return acc_all, acc_bins, f1_score_cls, conf_matrix
...
def main(_):
  ...
  acc_all, acc_bins, f1_score_cls, conf_matrix = eval_classifier(model,
                                                                 dataset,
                                                                 cls_bins)
  save_results(acc_all, acc_bins, f1_score_cls, conf_matrix)
