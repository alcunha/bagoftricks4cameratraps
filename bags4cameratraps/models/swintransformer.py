...
#
# Code from https://github.com/rishigami/Swin-Transformer-TF
...
class Mlp(tensorflow.keras.layers.Layer):
  ...
  def call(self, x):
    x = self.fc1(x)
    x = tensorflow.keras.activations.gelu(x)
    x = self.drop(x)
    x = self.fc2(x)
    x = self.drop(x)
    return x
...
class WindowAttention(tensorflow.keras.layers.Layer):
...
class DropPath(tensorflow.keras.layers.Layer):
...
class SwinTransformerBlock(tensorflow.keras.layers.Layer):
...
class PatchMerging(tensorflow.keras.layers.Layer):
...
class BasicLayer(tensorflow.keras.layers.Layer):
  def __init__(self, dim, input_resolution, depth, num_heads, window_size,
    ...
    # build blocks
    self.blocks = tensorflow.keras.Sequential([SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
...
class PatchEmbed(tensorflow.keras.layers.Layer):
...
class SwinTransformerModel(tensorflow.keras.Model):
  def __init__(self, model_name='swin_tiny_patch4_window7_224', include_top=False,
    ...
    # build layers
    self.basic_layers = tensorflow.keras.Sequential([BasicLayer(dim=int(embed_dim * 2 ** i_layer),
...
def SwinTransformer(model_name='swin_tiny_224', num_classes=1000, include_top=True, pretrained=True, use_tpu=False, cfgs=CFGS):
  ...
  net(tensorflow.keras.Input(shape=(cfg['input_size'][0], cfg['input_size'][1], 3)))
  if pretrained is True:
    url = f'https://github.com/rishigami/Swin-Transformer-TF/releases/download/v0.1-tf-swin-weights/{model_name}.tgz'
    pretrained_ckpt = tensorflow.keras.utils.get_file(
